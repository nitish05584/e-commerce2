...
const Products = () => {
  ...
  const {search,setSearch,categories,category,setCategory,
    totalPages,page,setPage,price,setPrice,products,loading
  }=ProductData()
  const clearFilter=()=>{
    setPrice("")
    setCategory("")
    ...
    <div className='flex flex-col md:flex-row h-full'>
      <div className={`fixed inset-y-0 left-0 z-50 md:z-40 w-64 bg-white dark:bg-gray-800 shadow-lg transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${show ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className='p-4 relative'>
          ...
          <div className='mb-4'>
            ...
            <select className='w-full p-2 border rounded-md dark:bg-gray-900 dark:text-white ' value={category} onChange={(e)=>setCategory(e.target.value)}>
      <option value="">All </option>
          ...
          <div className='mb-4'>
            ...
            <select className='w-full p-2 border rounded-md dark:bg-gray-900 dark:text-white ' value={price} onChange={(e)=>setPrice(e.target.value)}>
      ...
      <div className='flex-1 p-4'>
        <button onClick={()=>setShow(true)} className='md:hidden bg-blue-500 text-white px-4 py-2 rounded-md mb-4'><Filter/></button>
        {
