...
const Checkout = () => {
   ...
   try {
    const {data}=await axios.post(`${server}/api/address/new`,{address:newAddress.address,phone:newAddress.phone},{
      headers:{
        token:Cookies.get("token")
      }
    })
    ...
    <div className='container mx-auto px-4 py-8 min-h-[60vh]'>
      <h1 className='text-3xl font-bold mb-6 text-center'>Checkout</h1>
      {
        loading ? <Loading/>: <div className='grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 gap-4'>
            {address && address.length>0 ? address.map((e)=>(
             ...
             <Button variant="destructive" size="icon" onClick={() => handleDeleteAddress(e._id)}>
              <Trash/>
             </Button> 
                  </h3> 
     ...
     <div className='space-y-4'>
      <Input placeholder="Address" value={newAddress.address} onChange={(e) => setNewAddress({...newAddress, address: e.target.value})} />
       ...
       <Input type="number" placeholder="phone" value={newAddress.phone} onChange={(e) => setNewAddress({...newAddress, phone: e.target.value})} />
     </div>
