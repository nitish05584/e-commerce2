...
const Payment = () => {
    ...
    const paymentHandler=async()=>{
       if(method=="cod"){
        setLoading(true)
        try {
            const {data}=await axios.post(`${server}/api/order/new/cod`,{ method,phone:address.phone,address:address.address },{
                headers:{
                    token:Cookies.get("token")
                }
            })
    ...
    <div>
      {
        loading ?(<Loading/>):(<div className='container mx-auto px-4 py-8'>
         <div className='space-y-8'>
            <h2 className='text-3xl font-bold text-center'>Procced to Payment</h2>
            <div>
         <h3 className='text-xl font-semibold'>Products</h3> 
         <Separator className="my-2"/> 
         <div className='space-y-4'>
            {cart && cart.map((e,i)=>(
              <div key={i}  className='flex flex-col md:flex-row items-center justify-between bg-card p-4 rounded-lg dhadow border dark:border-gray-700'>
            ...
            {
                address && <div className='bg-card p-4 rounded-lg shadow border space-y-4 dark:border-e-gray-700'>
                  ...
                  <select value={method} onChange={e=>setMethod(e.target.value)} className='w-full p-2 border rounded-lg bg-card dark:bg-gray-900 dark:text-white'>
                ...
                <Button className="w-full py-3 mt-4" disabled={!method||!address||loading} onClick={paymentHandler}>Proceed to Checkout</Button>
            </div>   
        </div>
        )
      }
