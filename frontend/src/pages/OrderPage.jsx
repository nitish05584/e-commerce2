...
const OrderPage = () => {
    ...
    if(!order){
       ...
       <h1 className='text-2xl font-bold text-red-600'>No Order with this id Yet</h1> 
  ...
  return (
    ...
    <CardTitle className='text-2xl font-bold'>Order Details
    </CardTitle>   
     ...
     <span className={`${order.status==="Pending" ?"text-yellow-400":"text-green-500"}`}>{order.status}</span>   
    </p>
      ...
      <div className='grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6'>
        {
            order.items.map((e,i)=>(
              <Card key={i}>
                ...
                <img src={e.product.images[0]?.url} alt={e.product.title} className='max-w-full max-h-full object-contain'/>
                </Link>
                <CardContent>
